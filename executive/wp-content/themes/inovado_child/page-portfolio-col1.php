...
<?php	$portfolio_filters = get_terms('portfolio_filter', array('orderby' => 'none'));
		if($portfolio_filters): ?>
			<ul class="clearfix">
				<li><a href="#" data-filter="*" class="active"><?php _e('Todos', 'minti'); ?></a></li>	
			...
			<div class="<?php if($terms) : foreach ($terms as $term) { echo 'term-'.$term->slug.' '; } endif; ?>portfolio-item-one sixteen columns">
				...
				<?php if ( has_post_thumbnail()) { ?> 
					...
					<div class="portfolio-desc">
				  		...
				  		<?php the_excerpt(); ?>
				  		<a href="<?php the_permalink() ?>" class="button"><?php _e('Veja detalhes', 'minti'); ?></a>
						<?php if( get_post_meta( get_the_ID(), 'minti_portfolio-link', true ) != "") { ?>
						<a href="<?php echo get_post_meta( get_the_ID(), 'minti_portfolio-link', true ); ?>" target="_blank" class="button"><?php _e('Acesse', 'minti'); ?></a>
						<?php } ?>	
				  	</div>
				  	<?php echo $embedd; ?>
				<?php } ?>
