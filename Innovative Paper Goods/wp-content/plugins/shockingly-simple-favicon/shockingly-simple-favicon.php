...
// FUNCTION: filexiste($fileurl) - return TRUE if $fileurl exist
function filexiste($fileUrl) {
...
add_filter('plugin_action_links', 'favi_plugin_actions', 10, 2);
function favi_plugin_actions($links, $file){
global $favi_dom;
	static $this_plugin;
 	if( !$this_plugin ) $this_plugin = plugin_basename(__FILE__);
 	if( $file == $this_plugin ){
		$settings_link = '<a href="options-general.php?page=shockingly-simple-favicon/shockingly-simple-favicon.php">' . __('Settings', $favi_dom) . '</a>';
		$links[1] = $links[0];
...
$plug_site = 'http://www.incerteza.org/blog/projetos/shockingly-simple-favicon/';
	...
	if ( !filexiste($opt['url']) && ($opt['url'] != NULL || $opt['url'] != '') ) { echo '<div id="message" class="updated fade"><p><strong>' . __('Can\'t find the configured favicon.ico, using the default.', $favi_dom) . '</strong></p></div>'; }
?>
<div class="wrap">
