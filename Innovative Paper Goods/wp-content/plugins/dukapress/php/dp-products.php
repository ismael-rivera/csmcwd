<?php
// ISMAEL WAS HERE-----WITH MY OWN FUNCTIONS 
/*
...
function dpsc_pnj_grid_display($atts, $content=null) {
    ...
    if (is_array($products) && count($products) > 0) {
        $content .= '<div class="dpsc_grid_display"><table class="products_tbl" border="0" cellspacing="0" cellpadding="0"><tr>';
        $count = 1;
        $all_count = 0;
        foreach ($products as $product) {
            ...
            if ($output) {
                ...
                $content .= '<td class="unit"><div class="dpsc_grid_product">';
				$content .= '<table class="isma_prod_tbl" border="0" cellspacing="0" cellpadding="0">';
  $content .= '<tr>';
				$content .= '<td colspan="2" class="title"><p><a href="' . $prod_permalink . '" title="' .$product->post_title . '">' . __($product->post_title) . '</a></p></td>';
				$content .= '</tr><tr>';
                $content .= '<td colspan="2"><div class="dpsc_grid_product_image">';
                if ($main_image != '') {
                ...
                $content .= '</div></td>';
				$content .= '</tr><tr>';
                $content .= '<td class="availability" align="left"><p><strong>Here Goes The availability</strong></p></td>';
				$content .= '<td align="right"><p class="price">' . $product->price . '</p></td>';
                $content .= '<tr><td colspan="2"><p class="detail">' . mb_substr($product->post_excerpt, 0, 80)  . '</p></td>';
			   $content .= '</tr></table>';
				$content .= $output['start'];
                $content .= $output['add_to_cart'];
                $content .= $output['end'];
                ...
                if ($count === intval($column)) {
					...
					$content .= '</tr><tr><td colspan="5">&nbsp;</td></tr><tr>';
                    $count = 0;
                } else {
				$content .= "<td width='38'>&nbsp;</td>";	
					}
                $count++;
                $all_count++;
            }
